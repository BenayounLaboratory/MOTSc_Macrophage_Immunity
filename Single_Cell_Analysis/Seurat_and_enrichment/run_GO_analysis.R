...
cluster5   <- read.table('2021-07-08_cluster5_MarkersWilcoxon_FDR5.txt', header = T, sep = "\t")
cluster6   <- read.table('2021-07-08_cluster6_MarkersWilcoxon_FDR5.txt', header = T, sep = "\t")
background <- read.table('2021-07-08_All_BMDMs_scGenes_Background.txt', header = T, sep = "\t")

cluster5.pos <- rownames(cluster5)[cluster5$avg_logFC >0]   # 107
cluster6.pos <- rownames(cluster6)[cluster6$avg_logFC >0]   # 218
